...
db.query(`SELECT system_events.*, users.user_email, u.user_email AS client_email, user_settings.email_notifications AS user_email_setting, u.email_notifications AS client_email_setting, jobs.job_id, jobs.job_user, jobs.job_client, job_milestones.milestone_id, job_milestones.milestone_fund_due_date, job_milestones.milestone_payment_amount, job_milestones.charge_id, job_milestones.user_app_fee, job_milestones.app_fee_id FROM system_events
...
.then(result => {
    if (result && result.rows.length > 0) {
        for (let row of result.rows) {
            stripe.applicationFees.createRefund(row.app_fee_id, {
                amount: parseFloat(row.user_app_fee) * 100
            })
            .then(() => {
                stripe.refunds.create({
                    charge: row.charge_id,
                    amount: parseFloat(row.milestone_payment_amount) * 100,
