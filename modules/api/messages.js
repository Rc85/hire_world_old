...
app.post('/api/conversation/submit', (req, resp) => {
    if (req.session.user) {
        request.post('https://www.google.com/recaptcha/api/siteverify', {form: {secret: process.env.SEND_MESSAGE_RECAPTCHA_SECRET, response: req.body.verified}}, (err, res, body) => {
                            ...
                            try {
                                ...
                                if (sender && sender.rows[0].user_status === 'Active') {
                                    ...
                                    if (receiver.rows[0].allow_messaging && blocked.rows.length === 0) {
                                        ...
                                        if (receiver.rows[0].email_notifications) {
                                            let message = {
                                                to: receiver.rows[0].user_email,
                                                from: 'noreply@hireworld.ca',
                                                subject: 'You have a new message on HireWorld',
                                                templateId: 'd-c92a35b4849e4cf69b91cb24c1220e43',
                                                dynamicTemplateData: {
