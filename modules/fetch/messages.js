...
app.post('/api/get/messages/:type', async(req, resp) => {
    if (req.session.user) {
        let stages = ['Inquire', 'Active', 'Completed', 'Abandoned'];

        if (stages.indexOf(req.body.stage) < 0) {
                ...
                (async() => {
                    try {
                        ...
                        if (user && user.rows[0].user_status === 'Active') {
                            ...
                            let params = [req.session.user.username, req.body.offset, req.body.stage];
                            ...
                            let queryString = `SELECT jobs.*, unread.unread_messages, user_reviews.*,
...
app.post('/api/get/message', async(req, resp) => {
    if (req.session.user) {
        ...
        if (authorized !== undefined && authorized.rows.length === 1) {
            ...
            let job = await db.query(`SELECT * FROM jobs
            LEFT JOIN user_reviews ON user_reviews.review_job_id = jobs.job_id
            ...
            let messages = await db.query(`SELECT (
                ...
                AND CASE
                    WHEN message_sender = 'System' THEN message_recipient = $4 ELSE (message_recipient = $4 OR message_sender = $4)
                END
            ),
            messages.*, user_profiles.avatar_url, jobs.job_stage FROM messages
            LEFT JOIN users ON users.username = messages.message_sender
            LEFT JOIN user_profiles ON users.user_id = user_profiles.user_profile_id
            LEFT JOIN jobs ON jobs.job_id = messages.belongs_to_job
            WHERE messages.belongs_to_job = $1
            ${req.body.stage === 'Abandoned' ? `AND job_stage IN ($2, 'Incomplete')` : `AND job_stage = $2`}
            AND CASE
				WHEN message_sender = 'System' THEN message_recipient = $4 ELSE (message_recipient = $4 OR message_sender = $4)
			END
            AND messages.message_status != 'Deleted'
            ...
            .then(() => {
                if (messages && messages.rows.length > 0) {
                    resp.send({status: 'success', job: job.rows[0], messages: messages.rows, deleted: deleted});
                } else {
