...
app.post('/api/get/posted/jobs', authenticate, async(req, resp) => {
        await db.query(`SELECT * FROM job_postings WHERE job_post_user = $1 ORDER BY job_post_sector`, [req.session.user.username])
        .then(result => {
            if (result) {
...
app.post('/api/get/posted/job', async(req, resp) => {
    let application = await db.query(`SELECT * FROM job_post_applications WHERE applicant = $1 AND applied_job_post_id = $2`, [req.session.user.username, req.body.id])
    ...
    .then(async result => {
        ...
        let friend = await db.query(`SELECT friend_id FROM friends WHERE friend_user_1 = $1 AND friend_user_2 = $2`, [req.session.user.username, result.rows[0].job_post_user]);

        if (result) {
            resp.send({status: 'success', job: result.rows[0], application: application.rows.length > 0 ? application.rows[0] : null, isFriend: friend && friend.rows.length === 1, saved: saved.rows.length === 1, reported: reported.rows.length === 1});
        } else {
...
app.post('/api/get/saved/jobs', authenticate, async(req, resp) => {
    ...
    await db.query(`SELECT * FROM job_postings WHERE job_post_id = ANY($1)`, [savedIds])
    .then(result => {
        if (result) {
            resp.send({status: 'success', jobs: result.rows});
        } else {
