...
app.post('/api/user/settings/profile/save', authenticate, (req, resp) => {
    db.connect((err, client, done) => {
        ...
        if (req.body.first_name && !validate.nameCheck.test(req.body.first_name)) {
            resp.send({status: 'error', statusMessage: 'Invalid characters in name'});
        } else if (req.body.last_name && !validate.nameCheck.test(req.body.last_name)) {
            resp.send({status: 'error', statusMessage: 'Invalid characters in name'});
        } else if (req.body.businessName && !validate.businessNameCheck.test(req.body.businessName)) {
            ...
            (async() => {
                try {
                    ...
                    await client.query(`UPDATE user_profiles SET user_business_name = $1, user_address = $2, user_phone = $3, user_city_code = $4, user_country = $6, user_region = $7, user_city = $8, user_firstname = $9, user_lastname = $10 WHERE user_profile_id = $5 RETURNING *`, [req.body.businessName, req.body.address, req.body.phone, req.body.code, req.session.user.user_id, req.body.country, req.body.region, req.body.city, req.body.first_name, req.body.last_name])
                    .catch(err => {
                        throw err;
                    });
                    ...
                    .then(() => resp.send({status: 'success', statusMessage: 'Profile saved', user: user}));
                } catch (e) {
