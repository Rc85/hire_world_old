            ...
            if (this.state.type === 'card') {
                ...
                /* <React.Fragment>
                    ...
                    <InputWrapper label='Name on Card' className='mb-3' required>
                        <input type='text' name='name' id='nameOnCard' onChange={(e) => this.setState({name: e.target.value})} placeholder={this.props.config.typing ? '' : 'Your name on your profile will be used if left blank'} value={this.state.name} onFocus={() => this.props.dispatch(IsTyping(true))} onBlur={() => this.props.dispatch(IsTyping(false))} />
                    </InputWrapper>
