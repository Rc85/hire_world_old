...
const ListingRow = props => {
    ...
    if (props.listing.listing_purpose === 'For Hire') {
        purpose = <span className='mini-badge mini-badge-purple mr-2'>For Hire</span>
    } else if (props.listing.listing_purpose === 'Hiring') {
        purpose = <span className='mini-badge mini-badge-orange mr-2'>Hiring</span>
    }
    ...
    return(
        <div className='listing-row mb-3'>
            <div className='listing-row-main'>
                ...
                <div className='listing-row-title' title={props.listing.listing_title}>{purpose} <NavLink to={`/user/${props.listing.listing_user}`}>{props.listing.listing_title}</NavLink></div>
            ...
            <div className='listing-row-detail'>
                ...
                <div className='listing-row-detail-child'>
                    <FontAwesomeIcon icon={faDollarSign} className='text-special mr-1' /> {props.listing.listing_price} / {props.listing.listing_price_type} {props.listing.listing_price_currency}
                </div>

                <div className='listing-row-detail-child'>
                    <FontAwesomeIcon icon={faCheckCircle} className='text-special mr-1' /> <strong>{props.listing.job_complete}</strong> {parseInt(props.listing.job_complete) === 1 ? 'Job' : 'Jobs'} completed
                </div>
